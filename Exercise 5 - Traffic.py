...
d_Time_Traffic = defaultdict(int)
sum_Time_Traffic = 0
rec_time_count = 0

d_Traffic, sum_Traffic, reccount = querydb("select processedMessageandComment from comments c1 join postsummary p1 on c1.pid = p1.pid where category = 'Traffic';")

d_Time_Traffic, sum_Time_Traffic, rec_time_count = querydb("select createdTime from comments c1 join postsummary p1 on c1.pid = p1.pid  where category = 'Traffic' and createdTime >= '2013-01-01' and createdTime < '2015-01-01' ;")

rpm_p =  0.0
rpm_p = sum_Traffic/rec_time_count
...
for key in range(1,97):
    print 'record'
    print d_Traffic[key]
    print d_Time_Traffic[key]
    print rpm_p
    results_df[key] = (float(d_Traffic[key])/d_Time_Traffic[key])/rpm_p

text_file = open("D:\\Output_Exercise5_Traffic_Output.dat", "w")
